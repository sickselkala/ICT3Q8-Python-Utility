...
def cleartrackbuf(ser):
    '''
    Löscht alle internen Spurpuffer des Lesers als Vorbereitung zum nachfolgenden Schreiben
    '''
    txdata = b'C66'
...
def writetrackbuf (ser, track, trackdata):
    '''
    Beschreibt den Spurpuffer [track] im Leser mit den angegebenen Spurdaten [trackdata]
    '''
...
def writealltracks (ser):
    txdata = b'C7000' # SetWriteMode: 0=LoCo, 0=write-verify
    #print ("Schreibmodus konfigurieren: ")
    ict.sendrecvICT(ser, txdata)
    txdata = b'C79'
    return ict.sendrecvICT(ser, txdata)

def performancelog (ser):
    # Comando manuale: CfA (Performance Log)
    txdata = b'CfA' 
    return ict.sendrecvICT(ser, txdata)

def errorlog(ser, page=0):
    page_char = str(page).encode('ascii')
    cmd = b'CfC' + page_char
    return ict.sendrecvICT(ser, cmd)

def stampa_hex_dump(raw_data, titolo="Dati"):
    print(f"\n--- {titolo} ---")
    print(f"{'Data(Hex)':<53}    {'Data(ASCII)':<16}") #la stringa base è di 53 caratteri
    print("-" * 75)
    ...
    for i in range(0, len(raw_data), 16):
        ...
        #formattazione
        if len(chunk) > 8:
            # hex_raw[:23] sono i primi 8 byte (23 chars)
            # hex_raw[24:] è il resto saltando lo spazio centrale
            hex_final = f"  {hex_raw[:23]} - {hex_raw[24:]}  "
        else:
            # identazione ultima riga
            hex_final = f"  {hex_raw}"
        ...
        print(f"{hex_final:<53}    {ascii_part}")
